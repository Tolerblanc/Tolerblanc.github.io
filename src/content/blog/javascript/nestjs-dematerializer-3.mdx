...
lang: ko
author: Tolerblanc
updatedDate: 2025-02-16T00:00:00.000Z
description: NestJS가 제공하는 관점 지향 프로그래밍(AOP) 구현의 핵심
series:
  name: NestJS 해체분석기
  order: 3
---
...
지난 2편에서는 동적 모듈과 프로바이더 스코프에 대해 알아보았다. 

<LinkCard 
  href="https://tolerblanc.github.io/javascript/nestjs-dematerializer-2/" 
  title="NestJS 해체분석기 2 - 동적 모듈과 프로바이더 스코프" 
  description="환경에 따른 유연한 모듈 구성 방법과 인스턴스 생명주기 관리하기" 
/>

이번에는 NestJS의 실행 흐름을 제어하는 세 가지 핵심 구성 요소인 미들웨어(Middleware), 가드(Guard), **인터셉터(Interceptor)** 를 살펴보려고 한다. 

## 실행 순서부터 제대로 이해하기

<Figure 
  src="https://github.com/user-attachments/assets/30768a2d-85b7-43fb-87f5-095b316737fa" 
  alt="NestJS 요청 처리 파이프라인" 
  caption="그림 1. NestJS에 하나의 HTTP 요청이 들어올 때의 처리 순서 (출처: 우아콘2023)" 
/>
...
<Steps>
#### 미들웨어 (Middleware)
가장 앞단에서 공통적으로 필요한 전처리를 수행 (예: 로깅, CORS 설정, Request Body 파싱 등)

#### 가드 (Guard)
인증/인가(Authorization)나 접근 권한 같은 “조건 충족 여부”를 검사해, 특정 라우트 실행을 막거나 통과시키는 관문

#### 인터셉터 - 전처리 (Interceptor - Pre-processing)
컨트롤러 진입 전 요청을 가로채 추가 로직 주입

#### 파이프 (Pipe)
컨트롤러에 도달하기 직전, 입력 데이터(Parameters, Body 등) 검증/변환을 담당

#### 컨트롤러 (Controller) & 서비스 (Service)
실제 비즈니스 로직을 수행하고 결과 반환

#### 인터셉터 - 후처리 (Interceptor - Post-processing)
응답을 변형하거나 추가 로직(예: 캐싱, 로깅, 성능 측정 등)을 주입

#### 예외 필터 (Exception Filter)
처리 과정에서 에러가 발생하면, 해당 에러를 가로채 적절히 변환해 응답
</Steps>

각 단계가 서로 다른 책임을 지닌다. 이번 글에서는 특히 미들웨어, 가드, 인터셉터에 집중하여 어떻게 NestJS에서 이 개념들을 구현하고, 실제로 어떻게 활용할 수 있는지 자세히 살펴보겠다.
...
<Tabs>
  <TabItem label="roles.decorator.ts">
    ```ts
  ...
  <TabItem label="admin.controller.ts">
    ```ts
