...
지난 3편에서는 NestJS의 실행 흐름을 제어하는 미들웨어, 가드, 인터셉터에 대해 알아보았다. 

<LinkCard 
  href="https://tolerblanc.github.io/javascript/nestjs-dematerializer-3/" 
  title="NestJS 해체분석기 3 - 미들웨어, 가드, 인터셉터" 
  description="NestJS의 요청 처리 파이프라인과 AOP 구현 핵심 요소들 이해하기" 
/>
...
<Steps>
#### 예외 발생
애플리케이션의 어느 지점에서든 예외가 throw됨

#### 예외 레이어 감지
NestJS의 예외 레이어가 예외를 감지

#### 필터 검색
해당 예외 타입에 맞춰 등록된 필터가 있는지 확인 
...
<Tabs>
  <TabItem label="business.exception.ts">
    ```typescript
  ...
  <TabItem label="user.exceptions.ts">
    ```typescript
...
<Steps>
#### 라우트 핸들러 요청 진입
클라이언트로부터 특정 라우트로 요청이 들어옴

#### 파이프 실행
NestJS는 파이프를 순차적으로 실행하며, `transform(inputValue, metadata)` 메서드를 통해 값 검증 또는 변환 로직을 수행

#### 결과 전달
파이프가 예외를 던지지 않으면 변환된(혹은 그대로인) 값을 핸들러에 전달
</Steps>
