...
lang: ko
author: Tolerblanc
updatedDate: 2025-02-02T00:00:00.000Z
description: NestJS의 확장성과 유연성을 책임지는 두 키워드에 대해
series:
  name: NestJS 해체분석기
  order: 2
---
...
지난 1편에서는 NestJS가 어떤 철학과 배경을 가지고 만들어졌는지, 그리고 모듈 시스템과 DI(의존성 주입)가 어떻게 돌아가는지를 살펴보았다. 

<LinkCard 
  href="https://tolerblanc.github.io/javascript/nestjs-demeterializer-1/" 
  title="NestJS 해체분석기 1 - NestJS의 철학과 모듈성" 
  description="NestJS의 등장 배경, 핵심 철학, 그리고 기본적인 모듈 구조와 DI 시스템 이해하기" 
/>

이번에는 한 걸음 더 깊이 들어가서, NestJS의 확장성과 유연성을 책임지는 두 가지 핵심 개념인 **동적 모듈(Dynamic Module)** 과 **프로바이더 스코프(Provider Scope)** 에 대해 구체적으로 알아볼 것이다. 동적 모듈은 환경별로 다른 설정이 필요하거나 테스트를 위해 특정 기능을 모킹(Mocking)해야 할 때 유용하다. 프로바이더 스코프는 인스턴스의 생명주기를 세밀하게 제어할 수 있게 해주어, 메모리 관리나 요청별 상태 관리에 있어 큰 도움이 된다.
...
<Tabs>
  <TabItem label="database.module.ts">
    ```typescript
    @Module({})
  ...
  <TabItem label="config.service.ts">
    ```typescript
    @Injectable()
...
<Tabs>
  <TabItem label="singleton.service.ts">
    ```typescript
    // 기본값
    @Injectable()
  ...
  <TabItem label="request.service.ts">
    ```typescript
  ...
  <TabItem label="transient.service.ts">
    ```typescript
  ...
  <TabItem label="scope-test.controller.ts">
    ```typescript
...
<Tabs>
  <TabItem label="email.service.ts">
    ```typescript
    @Injectable({ scope: Scope.TRANSIENT })
  ...
  <TabItem label="email.controller.ts">
    ```typescript
    @Controller("emails")
